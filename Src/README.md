...
MForge is a clean Visual Studio Code extension for MUMPS and VistA development. Current version: **0.4.4**.
...
npm run test
npm run package
code --install-extension mforge-mumps-vista-ide-0.4.4.vsix
```

Expected VSIX filename for version 0.4.4: `mforge-mumps-vista-ide-0.4.4.vsix`. The exact VSIX filename is safer than `*.vsix` if your shell does not expand wildcards.
...
- Avoid wildcard installs unless `mforge-mumps-vista-ide-0.4.4.vsix` exists.
