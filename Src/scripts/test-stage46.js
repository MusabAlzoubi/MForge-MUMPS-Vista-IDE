...
const remoteUri = new Uri('vscode-remote', '/ssh-remote+dev/workspace/XUP.m');
...
const texts = new Map([
  ...
  [localUri.toString(), 'START D EN^XUP D BUILD G EXIT S V=$$VALUE^ROUTINEB()\nBUILD Q\nEXIT Q'],
  ['vscode-remote:/ssh-remote+dev/workspace/ROUTINEB.m', 'VALUE() Q 1']
]);
  ...
  if (request === 'vscode') {
    return {
      ...
      workspace: {
        ...
        findFiles: async () => [remoteUri, new Uri('vscode-remote', '/ssh-remote+dev/workspace/ROUTINEB.m')],
...
const hover = await new MumpsNavigationHoverProvider(index).provideHover(localDoc, new Position(0, line.indexOf('XUP') + 1));
assert.notEqual(hover, null, 'Ctrl+Hover navigation hover is available for resolvable references');
