...
function todayForTemplate(): string {
...
export function registerTemplateCommands(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.commands.registerCommand('mforge.insertRoutineHeaderTemplate', insertRoutineHeaderTemplate),
    vscode.commands.registerCommand('mforge.insertPatchChangeBlockTemplate', insertPatchChangeBlockTemplate),
    vscode.commands.registerCommand('mumps.insertRoutineHeaderTemplate', insertRoutineHeaderTemplate),
    vscode.commands.registerCommand('mumps.insertPatchChangeBlockTemplate', insertPatchChangeBlockTemplate)
  );
}
...
export async function insertRoutineHeaderTemplate(): Promise<void> {
  ...
  const routineName = path.basename(editor.document.fileName).replace(/\.[^.]+$/u, '') || 'UJOROUT';
...
export async function insertPatchChangeBlockTemplate(): Promise<void> {
  ...
  const block = createPatchChangeBlockTemplate({ company: 'EHS', author, patch, date: todayForTemplate(), fixType, reason, scope });
  await editor.edit((editBuilder) => editBuilder.insert(editor.selection.active, block));
}
