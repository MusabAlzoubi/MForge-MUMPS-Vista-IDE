...
Expected VSIX filename for version 0.4.10:

```text
mforge-mumps-vista-ide-0.4.10.vsix
```
...
After installing or launching the extension, open a MUMPS routine in a WorldVistA/Hakeem container or workspace. With the defaults below, MForge detects common routine folders and rebuilds the routine index automatically:
...
Run **MForge: Show Routine Index Status** from the command palette to confirm detected paths and key routine status. Run **MForge: Save Detected Routine Paths To Settings** only if you want MForge to write the detected paths into `mforge.routineSearchPaths`.
...
| `Error: ENOENT no such file or directory, open '*.vsix'` | A VSIX was not created or the wildcard was not expanded. | Packaging did not complete. Do not run install before `npm run package` succeeds; then use `code --install-extension mforge-mumps-vista-ide-0.4.10.vsix`. |
...
Do not use wildcard install commands unless `mforge-mumps-vista-ide-0.4.10.vsix` exists.
